...
(define* (new-app/k f args #:key (kont prim:halt)
                    (name (new-id "kont-"))
                    (karg (new-id "karg-")))
  (make-app/k (list kont name karg) f args))
...
(define (cfs expr args el)
  ...
  (match expr
    (($ lambda/k ($ cps _ kont name karg) fargs body)
     ...
     (make-lambda/k (list kont name karg) fargs (cfs body args el)))
    (($ app/k ($ cps _ kont name karg) f e)
     ;;(format #t "cfs 1 ~a~%" expr)
     (make-app/k (list kont name karg) (cfs f args el) (cfs e args el)))
    (($ seq/k ($ cps _ kont name karg) exprs)
     ;;(format #t "cfs 2 ~a~%" expr)
     (make-seq/k (list kont name karg) (map (lambda (ee) (cfs ee args el)) exprs)))
    (else
  ...
  (match expr
    (($ lambda/k ($ cps _ kont name karg) fargs body)
     (if (null? (insec fargs old))
         (make-lambda/k (list kont name karg) fargs (alpha-renaming body old new))
         ;; new binding, don't rename more deeply anymore
         expr))
    (($ app/k ($ cps _ kont name karg) f e)
     ;;(format #t "alpha 1 ~a~%" expr)
     (make-app/k (list kont name karg) (alpha-renaming f old new)
                 (map (lambda (ee) (alpha-renaming ee old new)) e)))
    (($ seq/k ($cps _ kont name karg) exprs)
     ;;(format #t "alpha 2 ~a~%" expr)
     (make-seq/k (list kont name karg)
                 (map (lambda (ee) (alpha-renaming ee old new)) exprs)))
    (else
...
(define (beta-reduction expr)
  (match expr
    ...
    (($ lambda/k ($ cps _ kont name karg) args body)
     ;;(display "beta 2\n")
     (make-lambda/k (list kont name karg) args (beta-reduction/preserving body)))
    (($ branch/k ($ cps _ kont name karg) cnd b1 b2)
     (make-branch/k (list kont name karg)
                    (beta-reduction/preserving cnd)
                    (beta-reduction/preserving b1)
                    (beta-reduction/preserving b2)))
    (($ seq/k ($ cps _ kont name karg) e)
     (make-seq/k (list kont name karg) (map beta-reduction/preserving e)))
    ((? bind-special-form/k? bsf)
     (beta-reduction
  ...
  (match expr
    ...
    (($ branch/k ($ cps _ kont name karg) cnd b1 b2)
     (make-branch/k (list kont name karg)
                    ...
                    (beta-reduction/preserving b2)))
    (($ seq/k ($ cps _ kont name karg) exprs)
     (make-seq/k (list kont name karg) (map beta-reduction/preserving exprs)))
    (else expr)))
...
(define* (cps->expr cps #:optional (hide-begin? #t))
  (match cps
    (($ letfun/k _ fname fun body)
     `(letfun ((,fname ,(cps->expr fun))) ,(cps->expr body)))
    (($ letcont/k _ jname jcont body)
